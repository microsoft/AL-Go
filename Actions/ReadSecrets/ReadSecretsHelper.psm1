...
function ValidateSecrets() {
    $Secrets = $script:gitHubSecrets.PSObject.Properties.Name
    $ObsoletedSecrets = Get-Content -Path (Join-Path $PSScriptRoot "ObsoletedSecrets.json") | ConvertFrom-Json | ConvertTo-HashTable
    $foundError = $false

    $ObsoletedSecrets.Keys | ForEach-Object {
        $obsoletedSecretName = $_
        $obsoletedSetting = $ObsoletedSecrets[$obsoletedSecretName]

        # Check if the setting is present in the settings file
        if ($Secrets -contains $obsoletedSecretName) {
            PrintObsoleteMessage -SettingName $obsoletedSecretName -Message $obsoletedSetting.message -Type $obsoletedSetting.Type
        ...
        $Secrets | Where-Object { $_ -like $obsoletedSecretName } | ForEach-Object {
