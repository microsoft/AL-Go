...
function AnalyzeRepo {
    ...
    if (!$doNotCheckArtifactSetting) {
        $artifactUrl = Determine-ArtifactUrl -projectSettings $settings -doNotIssueWarnings:$doNotIssueWarnings
...
function Determine-ArtifactUrl {
    ...
    if ($projectSettings.additionalCountries -or $country -ne $projectSettings.country) {
        ...
        $atArtifactUrl = Get-BCArtifactUrl -storageAccount $storageAccount -accept_insiderEula -type $artifactType -country at -version "$($ver.Major).$($ver.Minor)" -select Latest
        Write-Host "Latest AT artifacts $atArtifactUrl"
        $latestATversion = $atArtifactUrl.Split('/')[4]
        $countries = Get-BCArtifactUrl -storageAccount $storageAccount -accept_insiderEula -type $artifactType -version $latestATversion -select All | ForEach-Object {
