...
$allProjects, $modifiedProjects, $projectsToBuild, $projectDependencies, $buildOrder = Get-ProjectsToBuild -baseFolder $baseFolder -buildAllProjects ($buildAllProjects -or $publishSkippedProjects) -modifiedFiles $modifiedFiles -maxBuildDepth $maxBuildDepth -token $token -supportsLinuxFastLane $supportsLinuxFastLane -baselineWorkflowRunId $baselineWorkflowRunId
if ($buildAllProjects) {
    $skippedProjects = @()
}
else {
