...
function Get-DeploymentEnvironmentVariables {
    ...
    Write-Host "Getting deployment environment variables for repository $repository with mask $mask"

    $headers = GetHeaders -token $token

    $environmentsURI = "$api_url/repos/$repository/actions/environments"
    $environments = (InvokeWebRequest -Headers $headers -Uri $environmentsURI).Content | ConvertFrom-Json
    ...
    $environments | ForEach-Object {
        ...
        $variablesByEnvironment[$environmentName] = @()
        $environmentVariablesURI = "$($_.url)/variables"
        $environmentVariables = (InvokeWebRequest -Headers $headers -Uri $environmentVariablesURI).Content | ConvertFrom-Json
        $environmentVariables | ForEach-Object {
            $name = $_.name
            Write-Host "  Name: $name"
            if ($name -like $mask) {
                Write-Host "  Variable: $name = $($_.value)"
                $variablesByEnvironment[$environmentName] += $_.value
            }
        }
    }

    return $variablesByEnvironment
}