...
After normal test execution, the action exports the kept-alive container event log again and overwrites `ContainerEventLog.evtx` in the project folder, so failure diagnostics include events generated by the separate test run. Capture also runs when no test app or result is produced. If tests fail and capture also fails, the capture problem is reported as a warning while the original test failure is preserved.
