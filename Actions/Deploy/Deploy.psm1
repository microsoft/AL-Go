...
<#
    .SYNOPSIS
        Track a PR deployment against the PR branch in GitHub.
    .DESCRIPTION
        When deploying a PR build via PublishToEnvironment from main, GitHub auto-creates a deployment
        record against the trigger ref (main). This function deactivates that record and creates a new
        deployment against the actual PR branch, so the PR shows correct deployment status.
    .PARAMETER repository
    ...
    .PARAMETER environmentUrl
        Optional URL for the deployed environment
    .PARAMETER deploymentSuccess
        Whether the deployment succeeded
    .PARAMETER token
        The GitHub token (needs deployments:write permission)
#>
function Set-PRDeploymentTracking {
    Param(
        [Parameter(Mandatory = $true)]
        [string] $repository,
        [Parameter(Mandatory = $true)]
    ...
    Write-Host "Tracking deployment for PR #$prId to $environmentName"

    # Get the PR branch ref
    $prRef = GetHeadRefFromPRId -repository $repository -prId $prId -token $token
    if (-not $prRef) {
        Write-Host "::warning::Could not determine PR branch for PR #$prId - skipping deployment tracking"
        return
    }
    Write-Host "PR #$prId branch: $prRef"

    # Deactivate auto-created deployments against the workflow trigger ref for this environment
    # The environment: key on the Deploy job auto-creates a deployment against github.ref (e.g. main),
    # which is misleading for PR deployments.
    try {
        $triggerRef = $ENV:GITHUB_REF_NAME
        if ($triggerRef -and $triggerRef -ne $prRef) {
            $deploymentsUri = "$apiBase/deployments?environment=$environmentName&ref=$triggerRef&per_page=5"
            $deploymentsResponse = InvokeWebRequest -Headers $headers -Uri $deploymentsUri
            $deployments = $deploymentsResponse.Content | ConvertFrom-Json
            foreach ($dep in $deployments) {
                Write-Host "Deactivating auto-created deployment $($dep.id) (ref: $($dep.ref))"
                $body = '{"state":"inactive"}'
                InvokeWebRequest -Headers $headers -Method 'POST' -Uri "$apiBase/deployments/$($dep.id)/statuses" -Body $body | Out-Null
            }
        }
    }
    catch {
        Write-Host "::warning::Could not deactivate auto-created deployment: $($_.Exception.Message)"
    }

    # Create a deployment record against the PR branch
    try {
        $deploymentBody = @{
            ref = $prRef
            ...
            auto_merge = $false
            required_contexts = @()
            description = "Deployed via PublishToEnvironment (PR #$prId)"
        } | ConvertTo-Json -Compress

        $deploymentResponse = InvokeWebRequest -Headers $headers -Method 'POST' -Uri "$apiBase/deployments" -Body $deploymentBody
        $deployment = $deploymentResponse.Content | ConvertFrom-Json

        if ($deployment.id) {
            Write-Host "Created deployment $($deployment.id) against branch $prRef"

            $deployState = if ($deploymentSuccess) { 'success' } else { 'failure' }
            $statusBody = @{
                state = $deployState
                environment = $environmentName
                description = "Deployed PR #$prId to $environmentName"
            }
            if ($environmentUrl) { $statusBody['environment_url'] = $environmentUrl }
            $statusBodyJson = $statusBody | ConvertTo-Json -Compress

            InvokeWebRequest -Headers $headers -Method 'POST' -Uri "$apiBase/deployments/$($deployment.id)/statuses" -Body $statusBodyJson | Out-Null
            Write-Host "Deployment status set to $deployState"
        }
        else {
            Write-Host "::warning::Failed to create deployment record for PR #$prId"
        }
    }
    catch {
        Write-Host "::warning::Failed to create PR deployment record: $($_.Exception.Message)"
    }
}
