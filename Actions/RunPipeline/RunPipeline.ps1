    ...
    if ($baselineWorkflowSHA -and $baselineWorkflowRunId -ne '0' -and $settings.incrementalBuilds.enabled -and $settings.incrementalBuilds.mode -ne 'modifiedProjects') {
        ...
        if (!$buildAll) {
            ...
            elseif ($settings.incrementalBuilds.mode -eq 'modifiedAppsAndDependingApps') {
                $skipFolders = @()
                Sort-AppFoldersByDependencies -appFolders $settings.appFolders+$settings.testFolders+$settings.bcptTestFolders -baseFolder $ENV:GITHUB_WORKSPACE -skipApps ([ref] $skipFolders) -selectSubordinates $modifiedFolders | Out-Null
