...
**Status (2026-08-07): live.** `StefanMaron/AL-Go-PTE` and `AL-Go-AppSource` are deployed at merge commit `7f21177b`. No managed repo has onboarded yet.

Full onboarding/revert/fork-sync runbook lives in the `linux-fast-lane-onboarding` skill (user-level, not in this repo) — use it (or point a fresh agent at it) instead of rediscovering this from scratch. Before trusting any "is X deployed/onboarded" claim from memory or a skill, verify against the actual repos (`gh repo view <owner>/AL-Go-PTE`, check the target repo's `templateUrl`) — this state changes as deploys/onboardings happen.
