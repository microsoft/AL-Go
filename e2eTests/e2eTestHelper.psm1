...
function RemoveRepository {
    ...
    if ($repository) {
        ...
        try {
            ...
            @((invoke-gh api -H "Accept: application/vnd.github+json" /$str/packages?package_type=nuget -silent -returnvalue -ErrorAction SilentlyContinue | ConvertFrom-Json)) | Where-Object { $_.PSObject.Properties.Name -eq 'repository' } | Where-Object { $_.repository.full_name -eq $repository } | ForEach-Object {
                Write-Host "+ package $($_.name)"
                # Pipe empty string into GH API --METHOD DELETE due to https://github.com/cli/cli/issues/3937
                '' | invoke-gh api --method DELETE -H "Accept: application/vnd.github+json" /$str/packages/nuget/$($_.name) --input -
            }
        }
        catch {
