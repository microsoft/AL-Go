### Modular build (PREVIEW): split RunPipeline into CreateDevEnvironment, PublishApps and RunTests

A new opt-in **`useModularBuild`** setting builds and tests a project using three composable actions — **CreateDevEnvironment**, **PublishApps** and **RunTests** — orchestrated by a new **BuildAndTest** action, instead of the classic monolithic **RunPipeline** action. This splits the pipeline into distinct create-environment, publish and test phases that share a single development container within the build job, driving the public BcContainerHelper cmdlets directly. Enabling `useModularBuild` enforces `useCompilerFolder = true` (compilation is decoupled from the container and produced by the existing CompileApps step). The feature is fully opt-in and non-breaking: when `useModularBuild` is off, the classic RunPipeline path is unchanged. A companion **`keepEnvironment`** setting keeps the development container running after the build/test for debugging. See [settings.md](Scenarios/settings.md#useModularBuild).
