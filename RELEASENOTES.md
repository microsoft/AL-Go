...
However, sometimes it is useful to avoid merging complex types. This can be achieved by specifying `resetSettings` property on a settings object. The purpose of the property is to reset the values of the specified settings, so that new values can be set on subsequent settings level. Read more at [resetSetings property](http://aka.ms/algosettings#resetSettings)
