        ...
        Get-ChildItem -Path $srcPath -Recurse -File -Force | ForEach-Object {
            ...
            if ($_.Name -eq "AL-Go-Helper.ps1" -and ($config.ContainsKey("defaultBcContainerHelperVersion") -and $config.defaultBcContainerHelperVersion)) {
                ...
                for($idx=0; $idx -lt $lines.count; $idx++) {
                    if ($lines[$idx] -match '^(\s*)\$defaultBcContainerHelperVersion(\s*)=(\s*)"(.*)"(\s*#\s*(.*))?$') {
                        $comment = if ($Matches[6]) { " # $($Matches[6])" } else { "" }
                ...
                if (-not $found) {
                    throw 'Could not find defaultBcContainerHelperVersion line in AL-Go-Helpers.ps1 matching "^(\s*)\$defaultBcContainerHelperVersion(\s*)=(\s*)"(.*)"(\s*#\s*(.*))?$"'
                }
            }
