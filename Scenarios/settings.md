...
| <a id="testIsolation"></a>testIsolation | Opts the project into partitioned test runs, so that selected test codeunits are executed by a custom test runner (matching their `RequiredTestIsolation` requirement) and everything else runs under a default runner. Read more at [Test Isolation](TestIsolation.md). | `{ "enabled": false, "defaultRunnerCodeunitId": 0, "partitions": [] }` |
