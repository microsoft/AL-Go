...
| <a id="customCodeCops"></a>customCodeCops | CustomCodeCops is an array of paths or URLs to custom Code Cop DLLs you want to enable when building. Relative paths are resolved against the project folder (the folder containing `.AL-Go/settings.json`). | [ ] |
...
| <a id="symbolsSource"></a>symbolsSource | **PREVIEW:** Where the Microsoft symbols and the AL compiler come from when compiling without a container.<br />**artifact** - download the Business Central artifact and take symbols, compiler and assemblies from it. This is the default and the existing behavior.<br />**nuGet** - resolve only the symbols this project actually depends on from Microsoft's public MSSymbols NuGet feed, and the AL compiler from nuget.org. No artifact is downloaded. See [SymbolsFromNuGet.md](SymbolsFromNuGet.md).<br /><br />`nuGet` requires **workspaceCompilation** to be enabled. It cannot be used when an app targets `OnPrem` or `Internal` (those may use .NET interop, and the NuGet feeds carry no service tier assemblies), nor when **vsixFile** is a download URL. In both cases the build fails with a message telling you to use `artifact`.<br /><br />With `nuGet`, the symbols staged are exactly the ones your apps depend on: the application dependency closure, plus every Microsoft app your app.json files declare a dependency on (the test toolkit for a test app, for instance). Non-Microsoft dependencies (an AppSource app, for instance) are resolved by app ID against **trustedNuGetFeeds** and, unless **trustMicrosoftNuGetFeeds** is `false`, Microsoft's public AppSourceSymbols feed - the same feeds a Windows-container build resolves them from. Project dependencies within the same repo still come from **appDependencyProbingPaths** and the project's own build output.<br /><br />With `nuGet`, **vsixFile** selects the compiler the same way it always did: `default` picks the AL version matching the Business Central version, `latest` the newest released compiler, `preview` the newest prerelease. | artifact |
...
| <a id="linuxFastLane"></a>linuxFastLane | When true, builds the project using the Linux BC fast lane ([StefanMaron/MsDyn365Bc.On.Linux](https://github.com/StefanMaron/MsDyn365Bc.On.Linux)) instead of the standard Windows container pipeline. This compiles from source, publishes to a Linux BC container, and runs AL unit tests — much faster than a Windows container build, but with no signing, no BCPT tests, no page scripting tests, no Deliver, and a small number of AL tests known to fail on Linux BC (see that repo's `KNOWN-LIMITATIONS.md`). BC version and country are still taken from the existing `artifact`/`country` settings. Combine with [ConditionalSettings](#conditional) (`branches`/`workflows`) to scope this to, for example, pull request builds only, or a dedicated test branch. | false |
...
- **buildModes** settings will be applied when building with buildModes matching the patterns
...
| <a id="runNumberOffset"></a>runNumberOffset | when using **VersioningStrategy** 0, 3, or 15, the current workflow run uses the GITHUB RUN_NUMBER as the build part of the version number as described under VersioningStrategy. The RUN_NUMBER is ever increasing and if you want to reset it, when increasing the Major or Minor parts of the version number, you can specify a negative number as runNumberOffset. You can also provide a positive number to get a starting offset. Read about RUN_NUMBER [here](https://go.microsoft.com/fwlink/?linkid=2217059&clcid=0x409) | 0 |
