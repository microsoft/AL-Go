...
| <a id="useEarliestArtifact"></a>useEarliestArtifact | Set this to true in order for AL-Go for GitHub to build a project using the earliest compatible Business Central artifact, determined by the highest application dependency from app.json in all apps in the project and the `applicationDependency` in project settings. |
