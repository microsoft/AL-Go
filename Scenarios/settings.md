...
| <a id="useModularBuild"></a>useModularBuild | **PREVIEW:** Setting useModularBuild to true builds and tests the project using the modular build actions (CreateDevEnvironment, PublishApps and RunTests) orchestrated by the BuildAndTest action, instead of the classic RunPipeline action. This splits the pipeline into distinct create-environment, publish and test phases. Enabling this setting **enforces** `useCompilerFolder = true` (compilation happens outside the container). The modular build is opt-in and non-breaking: when it is off, the classic RunPipeline path is used. | false |
| <a id="keepEnvironment"></a>keepEnvironment | **PREVIEW:** When using the modular build (**useModularBuild**), setting keepEnvironment to true keeps the development container running after the build and test have completed (instead of removing it), which is useful for debugging. | false |
