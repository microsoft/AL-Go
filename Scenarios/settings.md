...
| <a id="enableCopilotCodeReview"></a>enableCopilotCodeReview | If set to true, Update AL-Go System Files adds the Copilot PR Review workflows, which use the [BC-ALAgents](https://github.com/microsoft/BC-ALAgents) reviewer to review AL changes and post findings as pull request comments. The GitHub organization must allow Copilot CLI requests billed to the organization. See [Enable Copilot code review](CopilotCodeReview.md). Default is false. |
