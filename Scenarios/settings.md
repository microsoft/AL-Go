...
| <a id="buildEnvironmentName"></a>buildEnvironmentName | If you want to use a Business Central online environment for running tests, you need to set `buildEnvironmentName` to the Business Central environment name (from the Admin Center) for the environment you want to use. You also need to create a [buildAuthContext](https://aka.ms/algosecrets#buildAuthContext) secret.<br />**Note:** when setting `buildEnvironmentName` AL-Go for GitHub will automatically switch to use the compiler folder feature for building. See [more](https://aka.ms/algosettings#useCompilerFolder). |
