...
| <a id="useSubmodules"></a>useSubmodules | If your repository is using Git Submodules, you can set the `useSubmodules` setting to `"true"` or `"recursive"` in order to use these submodules during build workflows. If `useSubmodules` is not set, submodules are not initialized. If your Submodules resides in a private repository, you need to define a `SubmodulesToken` secret. Read [this](https://aka.ms/algosecrets#UseSubmodules) for more information. |
