...
| <a id="useWorkspaceCompilation"></a>useWorkspaceCompilation | **PREVIEW:** Setting useWorkspaceCompilation to true causes your pipelines to use workspace compilation for building apps. This uses the AL tool to compile all apps in the workspace in a single operation, which can improve build performance for repositories with multiple apps. Like **useCompilerFolder**, this is containerless compiling. **Note** this feature is in preview and may change in future releases. | false |
| <a id="workspaceCompilationParallelism"></a>workspaceCompilationParallelism | **PREVIEW:** The number of parallel processes to use for workspace compilation. This setting only applies when **useWorkspaceCompilation** is set to true. Increasing this value can speed up compilation on machines with multiple CPU cores. Set to -1 to use all available processes on the machine. | 1 |
