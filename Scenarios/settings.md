...
| <a id="buildEnvironmentName"></a>buildEnvironmentName | If you want to use a Business Central online environment for running tests, you need to set `buildEnvironmentName` to the Business Central environment name (from the Admin Center) for the environment you want to use. You also need to set the `buildAuthContextSecretName` to the name of the [buildAuthContext](https://aka.ms/algosecrets#buildAuthContext) secret containing an auth context for this environment. |
| <a id="buildAuthContextSecretName"></a>buildAuthContextSecretName | If you want to use a Business Central online environment for running tests, you need to set `buildAuthContextSecretName` to the name of the secret containing the [buildAuthContext](https://aka.ms/algosecrets#buildAuthContext) secret, which should be an auth context with access to the environment specified by `buildEnvironmentName`. |
