...
| <a id="CICDPullrequestBranches"></a>CICDPullRequestBranches | CICDPullRequestBranches can be specified as an array of branches (branch patterns). A Pull Request Build will be triggered when a PR is opened to a branch that matches one of the patterns in this list. You need to run the Update AL-Go System Files workflow for the change to take effect.<br />Default is [ "main" ] |
